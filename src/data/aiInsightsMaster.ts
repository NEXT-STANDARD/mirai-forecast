/**
 * 未来レーダー (MiraiRadar.com) - Gemini 3.7 Flash 深層カタリスト分析マスター
 * 自動生成ファイル (sync_polymarket_cron.mjs により更新)
 */

export interface AiInsightData {
  titleJa: string;
  summaryJa: string;
  whyMovedJa: string;
  keyCatalysts: string[];
  urgencyLevel: 'high' | 'medium' | 'low';
  lastUpdated: string;
}

export const AI_INSIGHTS_MASTER: Record<string, AiInsightData> = {
  "30829": {
    "titleJa": "ギャビン・ニューサムは2028年民主党大統領候補指名を獲得するか？",
    "summaryJa": "2028年候補としての確率は15%にとどまり、ハリス副大統領の後塵を拝しています。",
    "whyMovedJa": "カマラ・ハリス副大統領が2028年選挙で民主党候補となったことで党内基盤を固めており、カリフォルニア州知事のニューサム氏は次点にとどまります。",
    "keyCatalysts": [
      "8月19-22日 シカゴ開催の米民主党全国大会（DNC）",
      "2028年11月 米大統領選挙（民主党・共和党決戦）"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "31552": {
    "titleJa": "JD・バンスは2028年米大統領選挙で勝利を収めるか？",
    "summaryJa": "2028年大統領選勝利の確率は20%で、共和党の将来有力株と見なされています。",
    "whyMovedJa": "トランプ前大統領の副大統領候補に選出されたことでMAGA陣営の後継者筆頭に浮上し、2028年の共和党本選候補として急浮上しました。",
    "keyCatalysts": [
      "10月1日 CBS主催 副大統領候補テレビ討論会（バンス対ウォルツ）",
      "2028年11月 米大統領本選投開票挙本選投開票"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "31875": {
    "titleJa": "ドナルド・トランプは2028年共和党大統領候補に指名されるか？",
    "summaryJa": "2028年のトランプ再指名確率は2%と、憲法上の制約からほぼ否定されています。",
    "whyMovedJa": "2028年選挙で勝利した場合は憲法修正第22条（3選禁止）に抵触し、敗北した場合も高齢（82歳）のため引退が不可避と見られているためです。",
    "keyCatalysts": [
      "2028年11月 米大統領本選投開票挙本選結果",
      "2026年後半1月20日 次期米大統領就任式"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "45915": {
    "titleJa": "タルシシオ・デ・フレイタスは2026年ブラジル大統領選に勝利するか？",
    "summaryJa": "サンパウロ州知事の26年当選確率は0%と、出馬見送りが前提視されています。",
    "whyMovedJa": "ボルソナーロ前大統領の被選挙権剥奪後も後継本命視されますが、2026年はサンパウロ州知事再選を優先し大統領選回避の観測が強まっています。",
    "keyCatalysts": [
      "2026年10月 ブラジル大統領選挙第1回投票",
      "2026年後半上半期 右派政党自由党（PL）の大統領候補指名調整"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "89502": {
    "titleJa": "ビットコインは2026年に60,000ドルを割り込むか？",
    "summaryJa": "2026年の6万ドル割れ確率は50%と市場の意見が真っ二つに割れています。",
    "whyMovedJa": "4年周期の半減期サイクルに伴うポストバブル調整懸念と、米国の国家準備金化議論など構造的需要増の強弱材料が拮抗しています。",
    "keyCatalysts": [
      "2028年11月 米大統領選挙（民主党・共和党決戦）",
      "2026年後半3月 米FRBのバランスシート縮小（QT）終了判断"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "89519": {
    "titleJa": "イーサリアムは2026年までに3,000ドルへ到達するか？",
    "summaryJa": "中長期での3,000ドル回復期待は37%と慎重ながらも意識されています。",
    "whyMovedJa": "レイヤー2（BaseやArbitrum）へのトラフィック流出でメインネット収益が落ち込む一方、今後のステーキング利回り改善への期待が交錯しています。",
    "keyCatalysts": [
      "2026年第4四半期 イーサリアム大型アップグレード進捗発表",
      "2026年後半上半期 米暗号資産規制法案（FIT21）の上院審議"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "149589": {
    "titleJa": "統一ロシアは次期下院選で過半数の最多議席を獲得するか？",
    "summaryJa": "与党「統一ロシア」の首位獲得は71%と極めて堅固な予測です。",
    "whyMovedJa": "プーチン政権下での野党勢力排除とメディア統制が継続しており、ウクライナ軍事侵攻下における愛国プロパガンダが議席維持を後押ししています。",
    "keyCatalysts": [
      "2026年9月 ロシア統一地方選挙投開票",
      "2026年後半春 プーチン大統領による年次教書演説"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "411239": {
    "titleJa": "アビィ・アハメドはエチオピア首相の座を維持できるか？",
    "summaryJa": "アビィ首相の続投は97%と確実視され、政権基盤の安定が評価されています。",
    "whyMovedJa": "ティグレ紛争終結後の和平合意（プレトリア合意）維持に加え、IMFからの4年間で34億ドルの融資プログラム合意により政権支持が盤石化しています。",
    "keyCatalysts": [
      "2026年9月 IMF・世界銀行によるエチオピア経済支援レビュー",
      "2026年10月 エチオピア連邦議会新会期演説"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "481717": {
    "titleJa": "FRBは9月FOMCで50bpの大幅利下げを実施するか？",
    "summaryJa": "米景気後退懸念が後退し、50bpの大幅利下げ観測は完全に消滅しています。",
    "whyMovedJa": "米7月CPIが前年比2.9%へ鈍化したものの、7月小売売上高が前月比+1.0%と消費の底堅さを示したため、25bpの通常利下げが既定路線となっています。",
    "keyCatalysts": [
      "9月6日 米8月雇用統計（非農業部門雇用者数・失業率）",
      "9月11日 米8月消費者物価指数（CPI）発表",
      "9月18日 FOMC政策金利発表および経済見通し（SEP）公表"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "659671": {
    "titleJa": "パリ・サンジェルマンは26-27欧州CLで優勝できるか？",
    "summaryJa": "PSGの2027年CL制覇の確率は14%と現実的な中位水準にあります。",
    "whyMovedJa": "キリアン・エムバペのレアル・マドリード移籍に伴いチーム再編期に入ったものの、ルイス・エンリケ監督主導の若手補強路線が一定の評価を得ています。",
    "keyCatalysts": [
      "8月30日 欧州夏の移籍市場デッドライン",
      "9月17日 新フォーマットでのUEFAチャンピオンズリーグ開幕"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "660108": {
    "titleJa": "ホルムズ海峡の船舶航行は8月31日までに正常化するか？",
    "summaryJa": "8月末の正常化は1%と完全に否定され、地政学リスクの長期化が定着しています。",
    "whyMovedJa": "中東地域における軍事的緊張の緩和の兆しが乏しく、メジャー海運各社（マースク、ハパックロイド等）が喜望峰経由の迂回ルートを維持しています。",
    "keyCatalysts": [
      "8月25日 イスラエル・ハマス間停戦協議の進展発表",
      "8月31日 国際海事機関（IMO）の紅海航行安全特別報告"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "660109": {
    "titleJa": "ホルムズ海峡の通航状況は9月30日までに正常化するか？",
    "summaryJa": "9月末までの正常化確率はわずか6%と、長期の混乱が織り込まれています。",
    "whyMovedJa": "フーシ派による紅海・アデン湾での商船攻撃継続と、イラン革命防衛隊による拿捕リスクが海運会社の保険料高騰を招き航路変更が続いています。",
    "keyCatalysts": [
      "8月25日 ガザ停戦交渉の妥結可否",
      "9月中旬 米主導「プロスペリティ・ガーディアン作戦」合同部隊会合"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "699735": {
    "titleJa": "米国は2026年7月までにイラン海上封鎖の終了を発表するか？",
    "summaryJa": "対イラン海上封鎖の解除観測は0%であり、対決姿勢の固定化が示されています。",
    "whyMovedJa": "イランのウラン濃縮進展やロシアへのドローン供与が継続しており、米超党派での対イラン制裁強化方針が解除を不可能にしています。",
    "keyCatalysts": [
      "2026年9月 IAEA（国際原子力機関）定例理事会報告",
      "2026年後半10月 国連安保理決議2231に基づく対イラン制裁失効期限"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "780132": {
    "titleJa": "ビットコインは8月中に10万ドルを突破するか？",
    "summaryJa": "8月中の10万ドル到達は極めて非現実的との見方が市場で完全に定着しています。",
    "whyMovedJa": "米現物ETFへの資金流入が一服し、パウエルFRB議長のジャクソンホール講演を前にマクロ環境の不透明感が強まり、上値余地が限定的と判断されています。",
    "keyCatalysts": [
      "8月23日 パウエルFRB議長のジャクソンホール会合での基調講演",
      "8月30日 米7月個人消費支出（PCEデフレーター）発表"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "780133": {
    "titleJa": "イーサリアムは8月中に3,000ドル台を回復できるか？",
    "summaryJa": "8月中の3,000ドル奪還はわずか2%と極めて悲観的な見通しです。",
    "whyMovedJa": "米現物イーサリアムETFからの資金流出（特にGrayscaleのETHE）が続き、DeFiプロトコルのガス代低迷によるバーン量減少が重石となっています。",
    "keyCatalysts": [
      "8月23日 ジャクソンホール会合での金融引き締め緩和シグナル",
      "8月30日 米7月PCEコアデフレーター発表"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "831375": {
    "titleJa": "イーロン・マスクの週間投稿数は20件未満に激減するか？",
    "summaryJa": "マスク氏のSNS発信頻度が極端に落ちる可能性はゼロと織り込まれています。",
    "whyMovedJa": "X（旧Twitter）上での活発な政治的発言やテスラ・SpaceX関連のポストが日常化しており、1週間に20ツイート未満にとどまる公算はありません。",
    "keyCatalysts": [
      "米大統領選に向けたマスク氏の公式Xスペース配信イベント",
      "テスラ「Robotaxi」発表イベント関連のティザー投稿"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "844164": {
    "titleJa": "イーサリアムは8月20日に1,400ドル以上をキープできるか？",
    "summaryJa": "1,400ドル割れのシナリオは100%排除され、市場は完全に安心しています。",
    "whyMovedJa": "現在2,600ドル近辺で推移しており、突発的なブラックスワンが発生しない限り1日足らずで45%以上の暴落は起こらないと確信されています。",
    "keyCatalysts": [
      "8月20日 現物イーサリアムETFの純資金流出入データ",
      "8月21日 米7月FOMC議事録発表"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "844167": {
    "titleJa": "ビットコインは8月20日時点で54,000ドル以上を維持できるか？",
    "summaryJa": "54,000ドル維持はほぼ確実と見なされ、下値支持線への信認が強固です。",
    "whyMovedJa": "直近の急落局面でも5万ドル台前半で機関投資家の押し目買いが確認され、現物ETFの純流出が止まったことで底堅い推移が続いています。",
    "keyCatalysts": [
      "8月20日 米主要現物ビットコインETFの日次資金フロー",
      "8月21日 米7月FOMC議事要旨の公表"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "851800": {
    "titleJa": "イーロン・マスクは8月下旬の1週間の投稿数を20件未満に抑えるか？",
    "summaryJa": "マスク氏がSNS発信を停止する可能性はゼロと判定されています。",
    "whyMovedJa": "xAIのGrokアップデートや米大統領選を巡る論争が激化しており、マスク氏の情報発信ペースが減速する余地はありません。",
    "keyCatalysts": [
      "xAI「Grok-2」正式リリース後のユーザーフィードバック",
      "テスラFSD（完全自動運転）v12.5の広域展開進捗"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "862400": {
    "titleJa": "ビットコインは8月21日時点で52,000ドルを上回っているか？",
    "summaryJa": "52,000ドル以上維持の確率は100%で、確固たる下値支持が確認されています。",
    "whyMovedJa": "6万ドル前後での保ち合いが続いており、8月5日の世界連鎖安時の安値（49,000ドル台）からのリバウンド基調が維持されているためです。",
    "keyCatalysts": [
      "8月21日 米労働省による年次基準改定（雇用統計ベンチマーク改定）",
      "8月21日 7月FOMC議事録公表"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "862401": {
    "titleJa": "イーサリアムは8月21日に1,400ドル以上を維持できるか？",
    "summaryJa": "1,400ドル以上の維持は99%確実とされ、暴落懸念はほぼ皆無です。",
    "whyMovedJa": "2,500ドル台の実勢価格に対し1,400ドルは昨年来の岩盤サポートラインであり、短期的な下振れリスクの範囲外と見なされています。",
    "keyCatalysts": [
      "8月21日 米労働省雇用統計年次改定値発表",
      "8月21日 イーサリアム現物ETF（ブラックロック等）の取引高推移"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "862998": {
    "titleJa": "ビットコインは8月23日までに78,000ドルへ急騰するか？",
    "summaryJa": "短期間での過去最高値更新は1%とほぼ不可能な水準と評価されています。",
    "whyMovedJa": "デリバティブ市場における建玉整理が進む中、短期間で2万ドル以上押し上げる現物買いモメンタムやマクロサプライズが欠落しています。",
    "keyCatalysts": [
      "8月21日 米7月FOMC議事録の公開",
      "8月23日 パウエルFRB議長ジャクソンホール講演"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "863010": {
    "titleJa": "イーサリアムは8月23日までに2,600ドルを上回るか？",
    "summaryJa": "直近での2,600ドル突破確率は3%と、戻りの鈍さが意識されています。",
    "whyMovedJa": "ビットコインに対するETH/BTC比率が0.04台前半まで下落しており、機関投資家の暗号資産アロケーションがビットコインに偏重しています。",
    "keyCatalysts": [
      "8月21日 米FOMC議事録公表による米金利動向",
      "8月23日 ジャクソンホールでのパウエル議長発言"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "871083": {
    "titleJa": "米・イラン間の停戦状態は8月22日まで維持されるか？",
    "summaryJa": "8月22日までの直接衝突回避は97%と極めて高い確率で織り込まれています。",
    "whyMovedJa": "カタール・エジプト仲介のガザ停戦交渉が継続中であり、イラン側もイスラエルへの報復攻撃のタイミングを慎重に計っているためです。",
    "keyCatalysts": [
      "8月21日 カイロでのガザ停戦・人質解放実務者協議",
      "8月22日 ブリンケン国務長官の中東歴訪総括会見"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  },
  "871229": {
    "titleJa": "ビットコインは8月20日に前日比プラスで終えられるか？",
    "summaryJa": "8月20日の日次上昇確率は80%と、短期的なリバウンドが期待されています。",
    "whyMovedJa": "直前の売り圧力が一服し、テクニカル的な自律反発局面に入ったことで、デイトレーダー中心に強気の買いポジションが構築されています。",
    "keyCatalysts": [
      "8月20日 ニューヨーク時間オープン時の米株市場動向（ナスダック）",
      "8月20日 米ビットコイン現物ETFの資金流入速報"
    ],
    "urgencyLevel": "high",
    "lastUpdated": "AI事前分析 (2026年8月最新)"
  }
};
