...
**最終更新**: 2026-08-23（Phase 1 第6回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **20項目 ALL PASS**、破壊テストは**7ラウンド連続合格**。
**N-21〜N-29 はすべて解決**し、本番は **128/128**。
ただし **N-30（全OGPが「世界の確率50%」）** が未解決で、これが現時点の最優先。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第6回）

直近レポート: `docs/phase1-verification-6-2026-08-23.md`（`.html` も同名であり）

**N-21〜N-29 はすべて解決。** 本番 128/128、ヘッダーは全幅正常、翻訳も同期をまたいで保持、
競技の誤分類は 13件 → 0件。**ただし最重要の N-30 が未解決です。**

### 🔴 未解決（優先順）

| ID | 内容 | 担当 | 実測 |
|---|---|---|---|
| **N-30** | **本番128銘柄すべてのOGP・og:description・JSON-LDが「世界の確率 50%」**。実際は 1%・41%・99% | ユーザー(DDL)＋実装 | 10銘柄比較で6件不一致・最大49pt・平均21pt |
| **N-31** | OGP画像の「SPREAD GAP」バッジが日本側の数字に**重なっている** | 実装 | 2枚とも同じ崩れ方 |
| 検査の穴 | 自己検証は `【世界の確率 \d+%】` の**形式**と PNG の**5KB以上**しか見ていない | 実装 | 128枚が誤値でも 20/20 PASS |
| テニス表記 | コロンの後に**空白が2つ**（24件） | 実装 | `clean.trim()` で解決 |
| 誤タイトルの自己修復 | ガードは誤った日本語タイトルも保持する。**手作業でしか直せない** | 実装 | 25件が同期以外の経路で更新された痕跡あり |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | ユーザー | 唯一残るインフラ項目 |

### N-30 の原因と直し方

`prerender_markets.mjs:102` と `generate_market_ogp.mjs:112` が
**存在しないカラム `event.world_prob_yes`** を読み、`undefined` なので**必ず 50 に落ちる**。

```
events のカラム: id, slug, title_ja, title_en, question_ja, question_en,
                 category, category_label, icon_url, end_date, is_active, created_at, updated_at
                 → world_prob_yes は無い
```

同期は**実際の確率をすでに計算している**（`sync_polymarket_cron.mjs:181-185` の `probYes`、
候補オブジェクトの `:296` にも入っている）が、**upsert のレコードに含めていない**だけ。

1. `events` に `world_prob_yes integer` を追加（DDL＝ユーザー作業）
2. upsert に `world_prob_yes: c.probYes` を追加
3. **`50` の無言フォールバックをやめる**（値が無ければ `—` と出す）

### このプロジェクトで繰り返している失敗の型

**「値が無いときに、それらしい値を静かに出す」**。これ一つで説明がつく：

- `.limit(100)`（N-20）— 超過分を黙って捨てる
- `_redirects` の無効行（N-23）— 無効でもエラーにならない
- `CF_PAGES_DEPLOY_HOOK` 未登録（N-24）— `if` で無言スキップ
- `world_prob_yes` 不在（N-30）— 無言で 50

設計判断は **「無いなら、無いと表示する」** を基準にする。

### 検証で効いた手順（第6回）

- **配信と中身は別に測る。** OGPを「200 / image/png / 5KB以上」で7ラウンド合格にしていたが、
  **画像を1枚も見ていなかった**。生成物は必ず1枚は目で見る（`Read` で PNG を開く）
- **対照を両方向で取る。** N-29 は「The International に非マッチ」だけでなく
  「Inter Milan にはマッチ」も確認した
- **`updated_at` は更新経路の指紋になる。** 新形式のタイトルなのに `updated_at` が古い
  ＝同期以外の経路で書かれた、と特定できた
