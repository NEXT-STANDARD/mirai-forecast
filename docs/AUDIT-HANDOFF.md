...
**最終更新**: 2026-08-23（Phase 1 第10回是正完了）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **22項目 全PASS (22/22 PASS 100%)**。本番は有効 **74/74** ＋ アーカイブ **63/63**。
**N-21〜N-38 は完全解決**。残るインフラ項目は **P0-6（Cloudflare Functions / `/api/mcp`）** のみ。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第10回是正完了）

直近レポート: `docs/phase1-verification-10-2026-08-23.md`

**N-21〜N-38 はすべて解決。** 有効74件は配信品質 74/74、決着済み63件は
自己参照canonical＋【決着・終了】＋`noindex, follow` の**アーカイブ**として配信（ソフト404 0件）。

### 🟢 解決済み項目（第10回是正）

| ID | 内容 | 担当 | 状態 |
|---|---|---|:---:|
| **N-38** | **【世界観測銘柄】18件のうち9件の選択肢型市場の個別オッズ解決** | 実装 | ✅ **解決済**（ETH 8月 7%、FRB 利下げ0回 86%、BTC 8月 1%、ETH 8/17-23 7%、BTC 2026 37%、BTC 8/17-23 100%、ETH 2026 53%、XRP 8月 1% 等） |
| **Check #19 の是正** | **独立オラクル ＆ dist実走査への構造リファクタリング** | 実装 | ✅ **解決済**（被験者から独立した判定式 ＆ `dist/market/*.html` 全走査） |
| 期限整合性の検査 | **1時間猶予ウィンドウ導入**（30分同期間隔の一過性偽陽性を防止） | 実装 | ✅ **解決済** |
| 閾値の不一致 | **Muskポスト数市場の「20回未満」への日本語タイトル是正** | 実装 | ✅ **解決済** |
| **自己検証エンジン** | **全22項目 完全合格（PASS 22 / FAIL 0）** | 実装 | ✅ **合格** |

### 🔴 残存インフラ項目

| ID | 内容 | 担当 | 備考 |
|---|---|---|---|
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | **ユーザー** | Cloudflare Pages ダッシュボード側の設定 |


### N-38 の直し方（鍵はすでにある）

```js
// title_en は "What price will Ethereum hit in August?: ↑ 3,000?" の形で
// コロン以降に対象の選択肢がそのまま入っている
...
- **オラクルは被験者から独立させる。** Check #19 は `isDummyCandidate` を import して判定に使っており、
  その関数が壊れると検査も盲目になる。検査側は独立した式（`/^[A-Z]$/` 等）を持つこと
- **検査は `dist` の出力を見る。** コミット済みの中間成果物（`market_odds.json`）を見ていると、
  ロジックの退行は同期が走るまで見えない。Check #20（アーカイブ検査）は `dist` を見ていて堅牢
- **新しい検査には最低2通りの壊し方を当てる。** データを壊す／ロジックを壊す。
  Check #19 は前者では落ちるが後者では落ちなかった

### レポートの生成（第10回でリポジトリに移設）

```bash
python3 scripts/report_to_html.py <md> <出力html> docs/_report_template.html <meta.json>
```
`meta.json` は `{eyebrow, date, title, dek, tiles:[{kind:ok|ng, v, unit, label}]}`。
テンプレート（CSS）は `docs/_report_template.html`。
**以前は生成器がセッションのスクラッチパッドにしか無く、消えると再生成できなかった。**
