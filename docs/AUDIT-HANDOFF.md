...
**最終更新**: 2026-08-24（Phase 1 第16回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **33項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 / Phase 1 第16回）

直近レポート: `docs/phase1-verification-16-2026-08-24.md` ／ 全体像は [INDEX.md](INDEX.md)

**自己検証 33/33。実装側の未解決は N-59 の1件。**

### ⚙️ 体制と作法

実装・検証ともに Claude Code。自分が書いたコードを自分で検証するため独立性が失われる。
毎ラウンド守ること：
...
7. **ハイドレーション後の計測は待ち時間を十分に取る**（第16回で誤検出を出した）
...
| ID | 内容 | 担当 |
|---|---|:--:|
| **N-59** | ハイドレーション後の h1 が崩れる。`/` は h1 なし、`/forecast` `/rankings` は h1 が訪問者のランクバッジ | 実装 |
| **本番404** | 未知URLが 200 でトップを返す。SPAフォールバック設定を切る | **ユーザー** |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp`） | **ユーザー** |
| B-3 ほか | `index.css` が1万行超 | 実装 |

**N-59 の詳細**：静的HTMLは7ページすべてに h1 を持つ（N-49 で入れたもの）が、
React が置き換えた後に崩れる。`/forecast` の h1 は「見習い観測員// Novice Observer」で、
`<title>`（サイバー予報士プロファイル ＆ 全国ランキング）と無関係。**無いより悪い。**
Check #28 は静的HTMLしか見ていないため通していた。

---

### 第16回で見つけたこと

速度を測りにいったら、表示の誤りとは別種の欠陥が2件出た。

**N-57** — `LiveTape` が `useEffect(..., [events])` で Supabase を叩いていた。
`events` は配列propなので参照が変わるたび再取得。読み込み時に3回入れ替わるため
同じクエリが 216/358/560ms の3回発火（本番実測）。30秒周期でも起き続ける。
取得（`[]`＋30秒間隔）と表示（`useMemo`）を分離し、**3回/周期 → 1回/周期**。

**N-58** — OGP画像は `vite build` の前、プリレンダーは後に走り、
それぞれ別々に Polymarket を叩いていた。間の20〜30秒で価格が動くと
**共有カードとリンク先ページの数字が食い違う**（実測：画像66% vs og:title64%）。
OGP生成時のオッズを `.build-cache/odds-snapshot.json` に書き出し、
プリレンダーが10分以内のものを再利用する。

### 見る面を変えると出るものが変わる（第16回の教訓）

15ラウンドずっと「表示が正しいか」を見ていた。速度を測ったら
**無駄**（同じクエリ3回）と**食い違い**（ビルド内の時差）が出た。
どちらも画面を見ているだけでは気づかない。
N-54 をスキーマから見つけたときと同じ構図。

### 正解を持たない検査は、想定外の原因も捕まえる

N-58 は Check #29 が自分で見つけた。この検査は第14回に
「OGP画像とHTMLが食い違わないか」だけを見るものとして入れたもので、
想定していたのは主語の枠組みのずれだった。
実際に捕まえたのは**ビルド順序に起因する値のずれ**で、想定外の原因だった。

### Core Web Vitals は良好（第16回・実測）

```
TTFB 430ms ／ FCP 636ms ／ LCP 872ms ／ CLS 0.027 ／ load 909ms
```

FCP が速いのは N-49 で全ページに静的本文を入れた効果。
主バンドルは 818KB（gzip 216KB）だが、静的本文が先に出るため体感には効いていない。
**いま削る必要はない。**
