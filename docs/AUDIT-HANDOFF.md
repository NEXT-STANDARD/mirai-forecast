...
次: **Loop 1** — state.json の週次差分から票速度を計算し、選定重み（curation_params.json に
外出し予定）を上限つきで自動調整＋探索枠1つ。総票数が少ないうちは最低サンプル数ガードで
調整を見送る設計にする。**Loop 3** — 自己診断の起票を入力に、定期セッションが週1件実装
（破壊テスト＋39項目＋PRゲート）。
