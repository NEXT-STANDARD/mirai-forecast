...
**最終更新**: 2026-08-25 朝（Phase 2-A 実装完了 ／ ユーザーのSQL実行待ち）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）→ Phase 2-A（絞り込み）実装まで。
自己検証エンジンは **37項目**（掲載/noindex一致を追加）。
**N-21〜N-52 は完全解決。** 残るのはユーザー操作の2件
（**本番404** の SPA フォールバック設定、**P0-6** Cloudflare Functions）と、
実装側の課題1件（**アプリ表示ロジックの自動回帰検査が無い** ＝ テスト基盤未整備）。
...
## 0. 【最新】いま最初に見るところ（2026-08-25 朝・その3）

**Phase 2-A（絞り込み）と 2-B（トラックレコード公開）まで本番反映済み。次は C: 埋め込み配布です。**
...
3. **E の設計** — 宿題2つをまとめて: track_record.json の週次自動再生成／
   同期で増える掲載数を20に保つ仕組み
...
| 内容 | 効果 |
|---|---|
| **Cloudflare Pages の SPA フォールバックを切る** | 本番404 と P0-6（MCP）が同時に直る |

（`patch_is_listed.sql` と push は 2026-08-25 朝に実施済み・本番反映を実測確認済み）
SPAフォールバックは急ぎではありません。ソフト404は canonical が `/` を指すため検索側では統合されています。
