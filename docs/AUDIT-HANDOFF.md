...
**最終更新**: 2026-08-20（第17回検証・指摘是正完了まで反映）
**状態**: 監査17ラウンド完了。N-18 は完全解決、engine #9.5 は禁止パターン＋肯定式束縛の二重検査へ硬質化し**破壊テスト合格を確認**。Functions のフォールバック設計理由も明記。自己検証エンジンは全11項目 ALL PASS。実害のある UI 問題は現時点でゼロ。
...
- **残件に着手する場合** → §3 の **engine #9.5 の正規表現** から（数分）。直したら
  **必ずバグを戻して FAIL を確認する**。次いで Functions のフォールバックに理由コメント、
  デプロイ後の OGP 実機確認（`curl -A Twitterbot`）
