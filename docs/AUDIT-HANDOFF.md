...
**最終更新**: 2026-08-23（Phase 1 第11回検証まで反映）
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第11回）

直近レポート: `docs/phase1-verification-11-2026-08-23.md` ／ 全体像は [INDEX.md](INDEX.md)

**N-21〜N-38 はすべて解決。** 有効74/74・アーカイブ63/63・自己検証22/22。
**本番から「50%」が完全に消えました**（確率42件・25種、ちょうど50%は0件）。

### ⚙️ 体制変更（2026-08-23〜）実装も Claude Code が行います

Gemini 3.7 Flash（Antigravity）のレートリミットが近づいたため、**Phase 1 の残りは
実装・検証ともに Claude Code が担当**します。これまでの「Gemini が実装 → Claude が検証」
という分担は一旦終了します。

**そのため、これまで機能していた独立性が失われます。** 自分が書いたコードを自分で検証すると、
「意図どおり動いた」ことの確認になりがちで、**意図そのものの誤り**を見落とします。
第10回の Check #19（検査が検査対象の関数を借りていた）と同じ構造です。

対策として、各ラウンドで以下を守ります。

1. **実装と検証を別ターンに分ける。** 実装直後に判定しない
2. **破壊テストは最低2通り**（データを壊す／ロジックを壊す）。第11回はこれで空振りを発見した
3. **本番の実測を必ず経由する。** ローカルのビルド結果だけで合格にしない
4. **独立したサブエージェントにレビューさせる。** 実装意図を伝えずに差分だけ渡す
...
| ID | 内容 | 担当 | 実測 |
|---|---|:--:|---|
| **og:title の本命名** | 【世界本命 62%】に**誰の62%か**が無い。読者にも伝わらず、検査の対象も存在しない | 実装 | 1つ直せば製品と検査の両方が直る |
| **Check #19 の dist 走査** | 抽出しているのが**日本語タイトル**なので永久に非マッチ＝空振り | 実装 | `leaderName` を `"C"` に強制しても **22/22 PASS** |
| **N-39** | マスク「40件未満」は `<40` が **35%** で存在するのに抑制 | 実装 | `title_en` にコロン接尾辞が無い形式（形式B）が未対応 |
| WTI のタイトル | 英語 `…?: ↑ $150?` に対し日本語が「到達水準予測」と一般化 | 実装 | 74件走査してこの1件のみ。タイトルを直せば 0% が出せる |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | ユーザー | 唯一残るインフラ項目 |

### title_en の2形式（N-39 の要点）
...
- **オラクルは被験者から独立させる**（第10回）→ 実装済み
- **検査は `dist` の出力を見る**（第10回）→ 形は入ったが、**見る先に検査対象が無かった**
- **新しい検査には最低2通りの壊し方を当てる。** ロジック破壊だけでは「出力が変わったのに PASS」で止まり、
  **値の強制注入**で初めて「そもそも出力に本命名が無い」と分かった
- **検査の欠陥と製品の欠陥が同じ場所にあることがある。** 今回の本命名がそれで、1つ直せば両方直る
