...
**最終更新**: 2026-08-20（第14回検証・指摘全件是正完了まで反映）
**状態**: 監査14ラウンド完了。個別ページの横はみ出しは全69ページで解消。第14回で出た新規6件（N-10〜N-15）および自己検証エンジンの破壊テスト適合（#9/#3）も全件解決済み。
...
| ID | 内容 | 場所 | 実施した是正・実測 |
|---|---|---|---|
| **N-11** | 埋め込みウィジェット固定値 | `EmbedWidgetPage.tsx` | ✅ `fetchLivePolymarketMarkets()` 接続。全69銘柄で実測日本世論・実オッズ・日本語タイトルをリアルタイム反映 |
| **N-12** | 詳細ページの乖離 `n≥3` ガード欠落 | `MarketDetailPage.tsx` ほか全8コンポーネント | ✅ 全8面（詳細、モーダル、OGP、Watchlist、CSV、Embed等）に `n>=3` ガード & `(n=X)` 併記を完備 |
| **N-10** | `/ai-connector`（`/developers`）375px はみ出し | `index.css:7263` | ✅ `grid-template-columns: minmax(0,1fr)` ＋ `min-width:0` 適用で 375px/360px 横スクロール 0px |
| **N-14** | 暗号資産価格帯の英語表題再発 | `sync_polymarket_cron.mjs` / `polymarketService.ts` | ✅ 価格帯正規表現トランスレーター配備 ＆ 未翻訳英語タイトル時は `is_active: false` とする構造的安全装置を配備 |
| **N-13** | タップ44px未満（詳細ページ等） | `index.css:10036-` | ✅ `pointer: coarse` 44px 擬似要素拡張に詳細ページ・モーダル全ボタンを追加 |
| **N-15** | 320px でヘッダー右クラスタが切れる | `index.css` | ✅ 360px 以下のメディアクエリでヘッダー要素のパディング・ギャップを最適化（画面内完全収容） |
| **Engine** | 自己検証エンジン #9/#3 の破壊テスト不合格・走査不足 | `audit_self_check.mjs` | ✅ #9 をルール内正規表現検査、#3 を全8コンポーネント網羅走査に拡張（破壊テスト合格） |
