...
**最終更新**: 2026-08-24 深夜（Phase 1 完了 ／ Phase 2 再設定 ／ N-64・トラックレコード基盤まで）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **36項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 深夜）

**Phase 1 は完了しました。いまは Phase 2 の入口です。**
自己検証 36/36。実装側の未解決は N-62（Phase 2 送り）のみ。

### 🔜 明日いちばん最初にやること

1. **既存73銘柄 → 20銘柄への絞り込み**（下記「A の残り」）
2. そのあと **トラックレコードのUI**（データ基盤は完成済み）
...
### A の残り：既存73件の整理には DDL が要ります

**`is_active=false` にすると「決着・終了」と表示されます。**
決着していない銘柄をそれで隠すのは事実に反するので、使えません。

```sql
ALTER TABLE public.events ADD COLUMN IF NOT EXISTS is_listed boolean NOT NULL DEFAULT true;
```

`is_active`＝まだ走っているか ／ `is_listed`＝サイトに出すか、と分ける。
削除された銘柄のページは 404 にせず、**【観測対象外】＋noindex** で残すのが誠実
（「決着した」とも「存在しない」とも言わない）。**この SQL はユーザーが実行します。**
...
| **`is_listed` 列の追加** | A の絞り込みが着手できる |

急ぎではありません。ソフト404は canonical が `/` を指すため検索側では統合されています。
