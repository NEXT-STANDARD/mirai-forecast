...
**最終更新**: 2026-08-24（Phase 1 第15回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **32項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 / Phase 1 第15回）

直近レポート: `docs/phase1-verification-15-2026-08-24.md` ／ 全体像は [INDEX.md](INDEX.md)

**自己検証 32/32。実装側の未解決はありません。**
...
5. **列挙は人ではなく検査にやらせる**（第15回の教訓・後述）
6. **破壊テストには、実際に起きた壊れ方を再現する**（第15回の教訓・後述）
...
**N-56** — 埋め込みウィジェットが**他人のサイトで嘘の世論を表示**していた。
...
投票は記録されず、表示だけが読者のクリックで ±2% 動く（n は据え置き）。
「6人が52%」と表示していたが、6人は50%と言っていた。`+2` に根拠はない。
記録できた票だけを分子・分母の両方に反映する形に直した（n=6 → 7、50% → 57%）。

**N-55** — 世界確率を「YES」と直書きしている面が3つ残っていた
（EmbedWidgetPage / ForecastHubPage / OgpPreviewModal）。

### 列挙は人ではなく検査にやらせる（第15回の教訓）

同じ型を5ラウンド繰り返している。

```
第11回 og:title            → 第13回 OGP画像・アプリ → 第14回 JSON-LD・静的シェル
→ 第15回 埋め込み・予測ハブ・OGPプレビュー
```

第14回の所感に「記憶で数えている限り繰り返す」と書いた。
**そう書いた次のラウンドで、また3面取りこぼした。** grep で手作業で列挙したせいで
クライアント描画の面が落ちた。Check #32 に49ファイルを走査させて初めて漏れが消えた。

### 破壊テストには実際の壊れ方を再現する（第15回の教訓）

Check #32 の初版は同一行の `YES {…worldProbYes…}` しか見ていなかった。
だが実際の N-55 は別名経由（`const worldYes = item.worldProbYes` → `YES {worldYes}%`）で、
**検査を書く原因になった欠陥そのものを素通りしていた**。第11回の Check #19 と同じ構図。

「2通りの壊し方を当てる」を守っていても、2通りとも同じ形なら見逃す。
**退避しておいた修正前のソースを検査に食わせて**、初めて穴が見えた。
