...
**最終更新**: 2026-08-24（Phase 1 第12回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **28項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
**N-21〜N-49 は完全解決。実装側の未解決なし。** 残るのはユーザー操作の2件
（**本番404** の SPA フォールバック設定、**P0-6** Cloudflare Functions）。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 / Phase 1 第12回）

直近レポート: `docs/phase1-verification-12-2026-08-24.md` ／ 全体像は [INDEX.md](INDEX.md)

**自己検証 28/28。本番は sitemap 80URL 全数で配信品質・静的本文ともに未達0件。**
第11回で残っていた4件（og:title の本命名 / Check #19 の空振り / N-39 / WTIのタイトル）は
すべて解決済み。N-40（多肢サブタイトル不整合）も解消。

### ⚙️ 体制（2026-08-23〜）実装も Claude Code が行います

Gemini 3.7 Flash（Antigravity）のレートリミットのため、実装・検証ともに Claude Code が担当。
**そのため独立性が失われます。** 自分が書いたコードを自分で検証すると「意図どおり動いた」
ことの確認になりがちで、**意図そのものの誤り**を見落とします。対策として毎ラウンド守ること：

1. **実装と検証を別ターンに分ける。** 実装直後に判定しない
2. **破壊テストは最低2通り**（ロジックを壊す／値を壊す）。かつ**部分的に**壊して
   検査の粒度も見る（全滅しか検出できない検査は弱い）
3. **本番の実測を必ず経由する。** ローカルのビルド結果だけで合格にしない
4. **検査を足したら、その検査自身の弱点を探す。** 第12回は2つ見つかった（後述）

> ⚠️ 別のエージェントが同じ作業ツリーにコミットすることがあります（2026-08-24 に発生）。
> 並行作業させる場合は git worktree を分けてください。同時に `npm run build` を走らせると
> 同じ `dist/` を奪い合います。
...
| ID | 内容 | 担当 | 実測 |
|---|---|:--:|---|
| **本番404** | 未知URLが 200 でトップを返す。`404.html` は正しく生成・配信されているが使われない | **ユーザー** | `/404` は正常（h1「ページが見つかりません」＋noindex＋リンク11本）。Cloudflare Pages の SPA フォールバック設定を切る必要がある |
...
**実装側の未解決はありません。** 残る2件はどちらも Cloudflare Pages のダッシュボード操作です。

### 緩和されている点（放置してよい理由）

未知URLのソフト404は canonical が `https://mirairadar.com/` を指すため、
検索エンジン側では**トップに統合されます**。実測で3パターンとも確認済み。
設定を直せば本物の404になりますが、直すまでの間も重複ペナルティは回避されています。
...
sitemap の 79URL 中 **78URL が本文0文字・内部リンク0本**で届いていた。

唯一の例外がガイド記事で、これは P1-3 で**個別に**直したもの。
**同じ欠陥がサイト全体に残っていることに、11ラウンド気づいていなかった。**

```
修正前 : <body><div id="root"></div></body>              ← 78/79 URL
修正後 : 本文 + 内部リンク（中央値11本）+ h1 + サイト内ナビ  ← 80/80 URL
```

アプリは `createRoot`（`hydrateRoot` ではない）なのでマウント時にコンテナを空にする。
つまり `#root` に静的本文を置いても不整合は起きず、起動後に置き換わるだけ。実機で確認済み。

### 検査を足したら、検査自身の弱点を探す（第12回の教訓）

Check #28 を書いた直後、**破壊テストBを設計する過程で自分の検査の穴が2つ見つかった**。

| 穴 | 症状 | 直し方 |
|---|---|---|
| ナビの文字列で文字数を満たす | h1も本文も空のページが合格する | `<nav>` を除いてから数える ＋ h1 必須にした |
| シェルに足した `<style>` のCSS | CSS文字列が本文としてカウントされる | `<style>` も `<script>` と同様に除去 |

2つ目は、**別の理由（見た目の改善）で入れた変更が、自分の検査を黙って弱めていた**もの。
検査を書いたあとに製品を触ったら、検査をもう一度壊してみる必要があります。
