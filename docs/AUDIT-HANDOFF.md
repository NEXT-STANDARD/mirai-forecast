...
## 0. 【最新】いま最初に見るところ（2026-08-25 朝）

**Phase 2-A（絞り込み）は実装完了。ボールはユーザー側にあります。**

### 🔜 次にやること（順番厳守）

1. **ユーザー**: Supabase SQL Editor で `scripts/patch_is_listed.sql` を実行
   （DDL＋48件の非掲載化。提案の中身と編集判断ポイントは
   [shiborikomi-teian-2026-08-25.md](shiborikomi-teian-2026-08-25.md) の「編集メモ」）
2. **ユーザー**: main へ push してデプロイ（アプリの is_listed 対応はコミット済み・DDL前でも無害）
3. そのあと **B: トラックレコードのUI**（データ基盤は完成済み）

> ⚠️ **順番が重要です。** トラックレコードの見出し数字は母集団で大きく変わります
> （スポーツ込み52% ／ スポーツ除外93%）。**絞り込み → 母集団確定 → 数字を出す**の順で。
...
| 内容 | 効果 |
|---|---|
| **`scripts/patch_is_listed.sql` の実行**（Supabase SQL Editor） | A の絞り込みが発効する（実装は済み） |
| **main への push（デプロイ）** | is_listed 対応が本番へ。DDL 前に出ても無害 |
