...
## 0. 【最新】いま最初に見るところ（2026-08-25 朝・その2）

**Phase 2-A（絞り込み）は本番反映まで完了。次は B: トラックレコードのUI です。**

2026-08-25 朝、ユーザーが `patch_is_listed.sql` を実行 → push → デプロイ済み。本番実測：

- sitemap **20 URL**（掲載銘柄と完全一致・死にURL 0）
- 非掲載47件：`noindex` ＋ 静的本文とアプリ両方に【観測対象外】注記、404にはしない
- 掲載ページ：robots meta なし、一覧から非掲載銘柄が消えたことを実描画で確認
- 自己検証 **37/37 PASS**（DDL適用後の実データ）

### 🔜 次にやること

1. **B: トラックレコードのUI**（データ基盤は完成済み・母集団は確定した）
   - まず `node scripts/build_track_record.mjs` を**絞り込み後の視点で再実行**し、
     見出し数字を確定させる（スポーツ1試合は掲載から消えたので「93%」側が本線）
   - 決着済み銘柄の扱い：track_record は過去の決着銘柄から作る。**is_listed で
     絞るか、全決着銘柄で出すかをまず決める**（絞ると n が減る。誠実さと両立させる）
2. C: 埋め込み配布（A完了で解禁）

> ⚠️ 見出し数字は母集団の定義で 52%↔93% まで動きます。UIに数字を置く前に
> 「どの母集団の的中率か」を1行で書ける状態にすること。
