...
**最終更新**: 2026-08-24（Phase 1 第14回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **31項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 / Phase 1 第14回）

直近レポート: `docs/phase1-verification-14-2026-08-24.md` ／ 全体像は [INDEX.md](INDEX.md)

**自己検証 31/31。実装側の未解決はありません。** 残るのはユーザー操作の3件。
...
5. **直す場所は思い出さずに列挙する**（第14回の教訓・後述）

> ⚠️ 別のエージェントが同じツリーにコミットすることがあります。衝突は起きていませんが、
> **未コミットの変更は向こうの `git add -A` に巻き込まれます**。小刻みにコミットするか、
> worktree を分けてください。

---

### 🔴 未解決（すべてユーザー操作・Cloudflare の設定2件のみ）

| ID | 内容 | 実測 |
|---|---|---|
| **本番404** | 未知URLが 200 でトップを返す。SPAフォールバック設定を切る | `/404` 自体は正常。canonical が `/` を指すため重複ペナルティは回避済み |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp`） | 本番404と同根の可能性 |

実装側の残りは `index.css` の分割（B-3、1万行超）のみで、機能影響はありません。

---

### 第14回で見つけたこと

**N-53** — 第11回に og:title、第13回に OGP画像とアプリを直したのに、
**JSON-LD と静的シェルだけが「YES」のまま**残っていた。AI連携を掲げる以上、
機械可読な面の嘘のほうが重い。同じ判断を3か所で別々に書いていたのが原因で、
`probSubjectLabel` に巻き上げて4面が共有するようにした。

**N-54** — 同一人物が何度でも投票できた。表示のバグではなく**データの信頼性**の問題。

```
japan_vote_logs に利用者を識別する列が無い（重複を後から取り除けない）
実データ48件中、2.2秒差の YES→YES が1組
本番UIの投票ボタン64個すべてに disabled が無かった
```

クライアント側（絞り込みガード＋disabled＋銘柄ごとの匿名キー）に加え、
**DDL も同日中に適用済み**。適用後に4経路を本番DBで実測し、
同一キーの2回目が `23505` で弾かれること、別キーとキー無しは通ることを確認した。
localStorage を消せば回避できる点は SQL ファイルに限界として明記してある。

### 直す場所は思い出さずに列挙する（第14回の教訓）

N-50 は **3ラウンドかけて1つの欠陥を追いかけた**。毎回「これで全部直した」と思っていた。

```
第11回 og:title を直す              → OGP画像・アプリ・JSON-LD・静的シェルが残る
第13回 OGP画像とアプリを直す        → JSON-LD・静的シェルが残る
第14回 grep で機械的に列挙してようやく全部
```

記憶で数えている限り同じことを繰り返す。**確率を描画している箇所を grep で列挙**して初めて漏れが見えた。

### 正解を持たない検査のほうが強いことがある

Check #30 は「正しい主語」を知らない。4面が食い違っていないかだけを見る。
正解を持つ検査は、その正解自体が古くなると一緒に腐る（N-52 の固定slug一覧がそれだった）。
Check #31 も同じ考えで、母集団を日付でなくデータ（`voter_key` の有無）で絞っている。

### UIを見ているだけでは出ない欠陥がある

N-54 は表示を見ていて気づいたのではなく、**スキーマの列を眺めていて「識別子が無い」**と
気づいたもの。そこから実データを見に行って連打の痕跡が出てきた。
**データの形を見る回**をもっと早く作るべきだった。
