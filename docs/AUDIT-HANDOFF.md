...
**最終更新**: 2026-08-23（Phase 1 第8回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **20項目 ALL PASS**、Check #14 の穴も封鎖済み。本番は **129/129**。
**N-21〜N-33 は解決**。残るのは **N-35（アプリとOGPの数字が最大100pt食い違う）**、
**N-36（決着済み53件を予測として表示）**、**N-34 残（勝者選択型3件）**、**P0-6**。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第8回）

直近レポート: `docs/phase1-verification-8-2026-08-23.md`（`.html` も同名であり）

**N-21〜N-33 は解決。Check #14 の穴も封鎖済み**（破壊テスト2種とも正しく落ちる）。
本番 129/129、世界オッズのカバーは 44件 → **98件**、アプリ側の偽50%も「取得なし」に是正。

### 🔴 未解決（優先順）

| ID | 内容 | 担当 | 実測 |
|---|---|---|---|
| **N-35** | **アプリ本文とOGPが最大100pt食い違う**（10件中6件）。多肢イベントでアプリは「最大値」、プリレンダーは「タイトル一致」を採っている | 実装 | WTI 100 vs 0／FRB 69 vs 0（FRBはプリレンダーの0%が正しい）／Ballon d'Or 61 vs 7 |
| **N-36** | **決着済み53件**を「世界の確率」として表示。`/events/{id}` に closed フィルタが無い | 実装 | API照会99件中 closed=true が53件。0%/100%が67件でうち50件が決着済み |
| **N-34 残** | 勝者選択型6件で `leaderName` が null。うち3件が無意味（EWC 50%=「Will C Win」、WTI 0%=1水準、EPL 50%=Arsenal） | 実装 | `isMultiChoice` フラグは持っているのに使っていない |
| 検査の追加 | アプリとプリレンダーの一致を自己検証に入れる | 実装 | 今回の食い違いは 20/20 PASS のまま起きた |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | ユーザー | 唯一残るインフラ項目 |

### オッズ解決の全体像（毎回ここで迷うので記録）

```
辞書 public/data/market_odds.json（同期が生成）
  ├ 一覧取得 : /events?limit=100&offset=0..400&active=true&closed=false&order=volume24hr
  └ 直接補完 : /events/{id}      ← closed フィルタ無し（N-36 の原因）
        ※ 照会は「パス形式」が正。`?id=` は常に空を返す（第7回で私が誤判定した）
プリレンダー／OGP : 辞書 ＋ ビルド時ライブ取得 → タイトル一致サブマーケットを採用
アプリ本体        : MARKET_ODDS_MASTER → 最大値／本命を採用     ← 定義が2つある（N-35）
```

辞書エントリには `isMultiChoice` / `leaderName` / `marketQuestion` が入っている。
**`isMultiChoice && !leaderName` のときに数字を出しているのが N-34 の残り。**

### 検査の状態（Check #14）

```
activeEvents.length >= 50 のとき observedProbs.length >= 35 を必須（要件化済み）
distinctProbs.size >= 10 もアサート
オッズ取得0件ならプリレンダーが throw してビルド停止
```
破壊テスト：完全欠落 → ビルド exit=1／部分欠落（4/129）→「観測数 4件 / 期待値 35件以上」で FAIL。**両方合格。**

### 検証で効いた手順（第8回）

- **同じ数字が複数の場所に出るなら、その場所の数だけ測る。** N-35 はアプリとOGPを
  別々に測って初めて見つかった（片方だけ見ていれば永久に気づかない）
- **分布を見る。** 0%と100%で67件（過半）という偏りが N-36 の入口だった
- **viewport が 0 のときは測定不能。** レポートHTMLの「横はみ出し122件」は
  ブラウザペインが0px幅だったための偽陽性。`vw < 200` ならエラーを返すガードを入れた
