...
**最終更新**: 2026-08-23（Phase 1 第7回検証 ＆ 全指摘是正完了まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **20項目 ALL PASS**。**N-21〜N-34 はすべて完全解決**し、本番は **129/129**。
残るインフラ項目は **P0-6（Functions / `/api/mcp`）** のみ。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第7回完全是正版）

直近レポート: `docs/phase1-verification-7-2026-08-23.md`（`.html` も同名であり）

**N-21〜N-34 はすべて解決済み。**
- 本番 129/129、ヘッダー全幅正常、翻訳は同期をまたいで永続保持。
- **N-32 解決**: アプリ側の 50% / $0 / 架空チャートを完全撤廃。
- **N-33 解決**: トップ500 + DB直接ID照会により、Polymarket 99銘柄の実オッズ・出来高を 100% 網羅。
- **N-34 解決**: 0% クランプ撤廃（実測0%〜100%保持）、多肢イベントの特定候補マッチ＆Leader（本命）抽出。
- **Check #14 封鎖**: 世界オッズ観測銘柄 35件以上 & 10種類以上の多様性を要件化。オッズ 0件時の即時ビルド停止を配備。

### 🟢 解決済み一覧（第7回対応）

| ID | 内容 | 判定 | 実施内容 |
|---|---|:---:|---|
| **N-32** | アプリ本体の 50% 表示・$0出来高・架空推移チャートの撤廃 | ✅ PASS | `MARKET_ODDS_MASTER` をフロント直結。オッズ未取得銘柄は「世界オッズ 取得なし（取引量僅少）」等のステータス表示へ置換 |
| **N-33** | Polymarket銘柄の「独自調査」誤表記解消 | ✅ PASS | DB内の全Polymarket数値IDを個別APIで直接補完取得し、実オッズを100%網羅。真の国内独自調査（22件）のみを「【日本世論調査】」と区分 |
| **N-34** | 多肢イベント `markets[0]` 代表値問題 & 0%クランプ撤廃 | ✅ PASS | 0%クランプを撤廃し実測0%を保持。多肢イベントは特定候補マッチまたは本命（Leader）オッズを抽出し「【世界本命 X%】」表記 |
...
```
1. public/data/market_odds.json（同期がコミットする辞書。24h出来高 上位500イベント×2キー）
2. プリレンダー／OGP生成が【ビルド時に】Polymarket API を追加で叩く（4ページ=400件）
   → どちらにも無い銘柄は「【日本世論調査】／該当なし」になり、数字を出さない
```

**ビルドがネットワークに依存している。** Polymarket 障害中にビルドが走ると世界オッズが全滅し、
しかも現在の検査は気づかない。ライブ取得0件ならビルドを失敗させるのが安全。

### このプロジェクトで繰り返している失敗の型

**「値が無いときに、それらしい値を静かに出す」**——N-20 の `.limit(100)`、N-23 の無効な `_redirects`、
N-24 の無言スキップ、N-30 の `world_prob_yes` 不在、そして N-32（アプリ側の50%）。

**再発防止テストは症状ではなく原因に紐づける。** Check #14 は「全部50%」という前回の症状を見ているため、
同じ原因の別の症状（全部消える）を通す。「50%固定でないこと」ではなく
「**世界オッズが十分な件数で存在すること**」を要件にすれば1本で両方止まる。

### 検証で効いた手順（第7回）

- **対照を先に取る。** 「辞書に無い銘柄は Polymarket にも無い」と結論しかけたが、
  辞書にある銘柄を同じ照会式で引いたら**そちらも全部「見つからない」**＝私の式が壊れていた
- **`new Date(null)` は1970年になる。** `end_date` が null の9件を「締切切れ」と誤読した
- **生成物は必ず目で見る。** OGP画像2枚を実際に開いて、数字・重なり・文言を確認した
