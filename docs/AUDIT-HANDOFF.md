...
### 🔴 ユーザー操作待ち（D 着手中・2026-08-25）

| 内容 | 効果 |
|---|---|
| **Cloudflare ダッシュボードの確認・SPAフォールバック解除** | 本番404 と P0-6（MCP）が同時に直る → D が解禁 |

**前提の罠は 2026-08-25 に解除済み**：`/embed/*` は静的ファイルが無く SPA フォールバック
だけで生きていたため、切ると配布済み埋め込みが全滅する状態だった。全114件（有効67＋
決着47）の静的シェルを prerender して本番反映済み（検査38項目め・破壊テスト済み）。
**全ルートが静的化されたので、フォールバックを切っても壊れる面は残っていない。**

切り分け済みの事実（2026-08-25 実測）：
- リポジトリ側は正常：functions/ コミット済み・`_routes.json`（/api/* のみ）配備済み・
  `_redirects` なし・404.html あり
- 本番：未知パスは **404.html の中身ではなく素の index.html を 200 で返す**
  ＝リクエストが Pages の not-found 処理に「不一致」として届いていない
- 残る容疑は2つ：①**ゾーン（mirairadar.com）のルール類**（Transform/Redirect/Page Rules・
  Workers Routes の catch-all 書き換え。SPA時代の遺物の可能性）②**Pages のビルド設定**
  （Root directory 誤り等で Functions が未コンパイル）。ダッシュボードは私から見えない
