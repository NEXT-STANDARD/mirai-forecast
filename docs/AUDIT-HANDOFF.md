...
**最終更新**: 2026-08-23（Phase 1 第4回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **20項目 ALL PASS**。**インフラ（N-21〜N-26）は全通過**し、本番は初の **125/125**。
残るのは **N-27（769〜1279pxのヘッダー崩れ）**、**英語タイトルの同期上書き**、**Check #18 の穴2つ**、**P0-6**。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第4回）

直近レポート: `docs/phase1-verification-4-2026-08-23.md`（`.html` も同名であり）

**インフラは全通過しました。** 本番は初の **125/125**、Deploy Hook は実際に発火、
`_redirects` は削除済み、ヘッダーと本文の縦グリッドは1px違わず一致。**N-21〜N-26 は解決。**

### 🔴 未解決（優先順）

| ID | 内容 | 担当 | 実測 |
|---|---|---|---|
| **N-27** | **769〜1279px でヘッダーが崩れ、更新ボタンが0%可視**。N-22 の修正が問題を上の帯へ移した | 実装 | 820px:はみ出し30件 / 1024px:18件 / 1280px:0件。必要幅が1271pxに固定される |
| **英語タイトル** | 8件中**6件が英語のまま**。原因は翻訳漏れではなく **同期による上書き** | 実装 | 英語6件の `updated_at` は全て最終同期時刻。翻訳が残った1件は同期に触られていない |
| **プレフィクス不整合** | `category=sports` なのにタイトルが「経済・市場予測:」 | 実装 | `prefix` はローカル `category`、DBは `finalCat` を使っている |
| **Check #18** | 穴が2つ。①クラスがメディアクエリにあれば基本ルールを消してもPASS ②`\b` がハイフン付き別名を通す | 実装 | 事故そのものが **20/20 PASS のまま再現**できた |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | ユーザー | 唯一残るインフラ項目 |
| 10px/11px文字 | マイ予報ボタン内。自分で引いた「12px未満」の指標に反する | 実装 | コントラストは合格（7.67 / 7.87） |

### N-27 の修正案（実測で確認済み・未適用）

原因は `hide-on-mobile`（畳んでよい要素）の境界が **768px** のままなこと。
769px以上でナビ・統計バッジ・提案・称号が一斉に出て、必要幅が **1271px** に固定される。

```css
/* .hide-on-mobile の指定だけを独立させ、境界を 1279px にする */
@media (max-width: 1279px) {
  .hide-on-mobile { display: none !important; }
}
@media (max-width: 768px) {   /* 既存ブロックからは .hide-on-mobile を外す */
```
→ **320〜1600px の全域**で更新ボタン可視100%・はみ出し0。ナビは1280pxで復帰。
トレードオフ：769〜1279px でナビ4本が非表示（現状その帯では触れないので実害は増えない）。

### 英語タイトルで踏んではいけない罠

`sync_polymarket_cron.mjs` は **毎回 `title_ja` をゼロから計算して `upsert(onConflict:'id')`** する。
DBの既存値を読まないので、**手でDBを直しても30分後の同期で戻る**。
直す場所は `DIRECT_MAP` / `translateFallback` のルール / upsert前に既存の日本語を優先する処理。

### Check #18 の直し方

- 穴②: `\.${cls}\b` → `\.${cls}(?![\w-])`（検証済み：`.stats-badges-slim-DISABLED` を正しく落とす）
- 穴①: 「**メディアクエリの外に基本ルールがあること**」を見る。
  必須12クラスのうち実セレクタが1箇所しかないのは4つだけで、残り8つは基本ルールを消してもPASSする
