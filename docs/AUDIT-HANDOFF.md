...
**最終更新**: 2026-08-24（Phase 1 第13回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **29項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 / Phase 1 第13回）

直近レポート: `docs/phase1-verification-13-2026-08-24.md` ／ 全体像は [INDEX.md](INDEX.md)

**自己検証 29/29。本番は sitemap 80URL 全数で未達0件。**

### ⚙️ 体制（2026-08-23〜）実装も Claude Code が行います

Gemini のレートリミットのため、実装・検証ともに Claude Code が担当。
**そのため独立性が失われます。** 毎ラウンド守ること：

1. **実装と検証を別ターンに分ける。** 実装直後に判定しない
2. **破壊テストは最低2通り**（ロジック／値）。かつ**部分的に**壊して検査の粒度も見る
3. **本番の実測を必ず経由する**
4. **検査を足したら、その検査自身の弱点を探す**
5. **測定方法が結論を作っていないか疑う。** 第13回は「5件の不一致」を検出したが、
   市場IDで突き合わせ直すと**0件**だった（question文字列が重複していた）

> ⚠️ 別のエージェントが同じ作業ツリーにコミットすることがあります。
> 並行作業させる場合は git worktree を分けてください。
...
| ID | 内容 | 担当 | 実測 |
|---|---|:--:|---|
| **本番404** | 未知URLが 200 でトップを返す。`404.html` は正しいが使われない | **ユーザー** | `/404` は正常。Cloudflare Pages の SPA フォールバックを切る必要がある。canonical が `/` を指すため重複ペナルティは回避済み |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | **ユーザー** | 本番404と同根の可能性 |
| **アプリ表示の回帰検査** | `src/utils/probabilityLabel.ts` に分岐を集約したが、テスト基盤（vitest 等）が無く自動検査ができない | 実装 | 第13回は実機計測で確認。導入は別判断 |
| B-3 ほか | `index.css` が1万行超。構造リファクタリング（A-7 / F-3 / D-4 / F-1） | 実装 | 機能影響なし |
...
```
テニス勝敗市場 outcomes: ["Lilli Tagger","Yulia Starodubtseva"]  prices: ["0","1"]
  → 0% は「Tagger が勝つ確率」
  → サイトは「YES 0% / NO 100%」と表示していた
```

判定文も「質問文『Tagger vs Starodubtseva』が公式に確認された場合、YESとして判定」
という、**確認しようのない文**になっていた。

実測（有効36件・**市場IDで一意に**突き合わせ）：`outcomes[0]=="Yes"` が34件、
`!= "Yes"` が2件。加えて日本語タイトルが問いになっていない銘柄（WTI「到達水準予測」／
EPL「優勝クラブ予測」）は、英語側が対象を名指ししているのに（`↑ $150` / `Arsenal`）
日本語に出ていなかった。

**分岐は `src/utils/probabilityLabel.ts` に集約した。** 散らばると直し漏れた箇所だけ
古い枠組みで残る。実際 og:title を第11回に直したあとも、OGP画像とアプリ本体は
「YES」のままだった。

#### N-52 — 検査が固定の slug 一覧を持っていた

N-38検査が8つの slug をハードコードしており、**銘柄が決着するたびに必ず落ちる**作りだった。
2026-08-24 04:00 UTC に1件が決着し、その40分後に赤くなった。
「有効かつ英語タイトルが対象を名指しする銘柄」から毎回導出するよう変更。

### 測定方法が結論を作る（第13回の教訓）

このラウンドで**3回、間違った結論を出しかけた**。

| 誤り | 何が起きたか | 気づいた理由 |
|---|---|---|
| `matchedVia` で言語を判別しようとした | 英語側の条件が先に評価されるため全31件が `en` になり、指標として無意味だった | 実測して `ja` が0件だったため |
| 「テニスは別の市場の数字を出している」 | 0%の市場が複数あり、**確率値で市場を特定した私のプローブ**が誤特定していた | `marketQuestion` で照合し直した |
| 「5件で価格が不一致」 | question文字列が重複しており、`find()` が別市場を拾っていた | **市場ID**で突き合わせ直したら0件だった |

3つとも「測り方が雑」なだけで、製品は正しかった。
**指摘を出す前に、その指摘が測定の産物でないかを確かめる。**
