...
**最終更新**: 2026-08-23（Phase 1 第9回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **20項目 ALL PASS**、Check #14 の穴も封鎖済み。本番は **77/77**。
**N-21〜N-33・N-35・N-36 は解決**。残るのは **N-34 残（プレースホルダ2件）**、
**N-37（非アクティブ53URLの跡地）**、**P0-6**。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第9回）

直近レポート: `docs/phase1-verification-9-2026-08-23.md`（`.html` も同名であり）

**N-21〜N-33、N-35、N-36 は解決。** 本番 **77/77**、アプリとOGPの乖離 **0pt**、決着済み銘柄 **0件**、
自己検証 20/20、Check #14 の穴も封鎖済み。有効銘柄は決着済み53件を外して **129件 → 77件**。

### 🔴 未解決（優先順）

| ID | 内容 | 担当 | 実測 |
|---|---|---|---|
| **N-34 残** | **プレースホルダ除外が発火していない**。「優勝チーム予測」に 50%（サブマーケット "C"）が出る | 実装 | EWC: `dbTitleEn="EWC 2026 CS2: Winner"` が `"c"` を含むため1文字の候補にマッチ。本命は Team Spirit 72% |
| **N-34 残** | レンジ選択型（WTI原油）で `↑ $150` = 0% を「世界の確率」として出している | 実装 | 26水準のうち1つ。最高値は `↓ $85` の100% |
| **N-37** | 非アクティブ化した**53URLが `canonical: https://mirairadar.com/` の200**を返す（トップの複製＝ソフト404） | 実装 | sitemapからは除外済み。OGP画像も古いまま残存 |
| 検査の追加 | 「プレースホルダ候補が選ばれていないこと」を自己検証へ | 実装 | 今回も 20/20 PASS のまま起きた |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | ユーザー | 唯一残るインフラ項目 |

### オッズ解決の構造（第9回で一本化された）

```
scripts/resolvePolymarketOdds.mjs  ＋  src/utils/polymarketOddsResolver.ts   ← 共通コア
  ↑ prerender_markets.mjs / generate_market_ogp.mjs / sync_polymarket_cron.mjs / polymarketService.ts

判定順:
 1. closed=true / active=false            → hasWorldOdds: false
 2. 単一マーケット                          → そのまま
 3. 多肢：タイトル一致（2-1）               → 見つかれば即 return ← ここが N-34 の原因
 4. 多肢：プレースホルダ除外して本命抽出      → leaderName つき【世界本命】
 5. 特定対象なしのレンジ                    → hasWorldOdds: false【世界観測銘柄】
```

**3 が 4 より先にあり、`lowerEn.includes(itemTitle)` が1文字でも通る。**
修正は「タイトル一致の候補集合から先にプレースホルダを除く」＋「英語側の部分一致に最小長」。

### ローカル起動（実行確認済み）

```bash
npm run dev                      # http://localhost:5173  Vite開発サーバー
npm run preview                  # http://localhost:4173  dist をローカル配信（プリレンダー確認用）
npm run build                    # sitemap → OGP → tsc → vite → prerender
node scripts/audit_self_check.mjs # 20項目の自己検証（npm run build を内包）
```

### 検証で効いた手順（第9回）

- **書いたものが正しくても、呼ばれなければ効かない。** N-34 のプレースホルダ除外は
  正規表現単体では正しくマッチする。**実データを1件通して出力を見る**まで分からなかった
- **削除の提案には、削除後の応答まで書く。** 「`is_active=false` にする」だけ書いたら、
  跡地の53URLがトップの複製を返す状態になった（N-37）
- **分布で異常に気づく。** 0%と100%が67件 → 15件に減ったことが N-36 解消の裏づけになった
