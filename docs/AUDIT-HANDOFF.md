...
**最終更新**: 2026-08-24（Phase 1 第17回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **34項目 全PASS**。本番は sitemap **80URL 全数**で未達0件。
...
## 0. 【最新】いま最初に見るところ（2026-08-24 / Phase 1 第17回）

直近レポート: `docs/phase1-verification-17-2026-08-24.md` ／ 全体像は [INDEX.md](INDEX.md)

**自己検証 34/34。実装側の未解決は N-61 の1件。**
...
8. **母集団を導出する検査には「新しいファイルを1つ足す」試験を当てる**（第17回）
9. **grep の件数は候補の数であって、欠陥の数ではない**（第17回）
...
| ID | 内容 | 担当 |
|---|---|:--:|
| **N-61** | ハイドレーション後に description と JSON-LD が既定値50%を捏造する（n<3 の53銘柄が該当）＋ 目視の1箇所「世界 NO 50%」 | 実装 |
| **本番404** | 未知URLが 200 でトップを返す | **ユーザー** |
...
クライアントの SEO ヘルパーが description を上書きし、JSON-LD（ItemPage）にも同じ値を入れる。
**値が既定値のときだけ嘘になる**（世界オッズと投票がある銘柄では正しく上書きされる）。
**Check #30 は静的HTMLしか見ていないため通していた。**
次ラウンドでは Check #30 をハイドレーション後にも届かせることも要る。

---

### 第17回の教訓：母集団を人の記憶から受け取らない

前回の宿題「面を取りこぼす型を止める仕組み」の答えは、
**検査自身が固定リストを持っていたこと**だった。
...
検査は「全8コンポーネント統一性」と名乗りながら母集団の4割を見ていなかった。
N-52（固定 slug 一覧）と同じ形。**あのとき直したのは1箇所だけで、
同じ形が他に残っていないかを探さなかった。**

実害：MainTradingChart が投票0件の銘柄で「有効投票数: 0票」と
「日本世論 YES: 50%」を並記していた。何ラウンドもかけて OGP とプリレンダーから
消した「無言の50%」が、固定リストの外側で生き残っていた。

**破壊テスト「新しいファイルを1つ足す」が効く。**
母集団が 12→13 に増え、その場で検出された。固定リストなら見えないままだった。
「いま正しいか」ではなく「**足したときに勝手に入るか**」を試す。
