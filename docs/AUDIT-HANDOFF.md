...
**最終更新**: 2026-08-23（Phase 1 第5回検証まで反映）
**状態**: 監査17ラウンド → 戦略ブレスト → Phase 0（配線）→ Phase 1（名乗り）まで進行。
自己検証エンジンは **20項目 ALL PASS**、破壊テストは**7ラウンド連続合格**。
**N-21〜N-27 はすべて解決**し、本番は **127/127**。残るのは **翻訳ルールの誤分類（N-28/N-29）**、
**英語銘柄の流入**、**P0-6**。
...
## 0. 【最新】いま最初に見るところ（2026-08-23 / Phase 1 第5回）

直近レポート: `docs/phase1-verification-5-2026-08-23.md`（`.html` も同名であり）

**インフラとヘッダーは片づきました。** 本番 **127/127**、ヘッダーは **320〜1920px の全域**で正常、
翻訳は**同期をまたいで保持**（24件が書込対象になってタイトル変更0件）、Check #18 の破壊テスト3種合格。
**N-21〜N-27 はすべて解決。**
...
| ID | 内容 | 担当 | 実測 |
|---|---|---|---|
| **N-28** | **MLB 9件・テニス4件に「欧州サッカー:」が付いている** | 実装 | 原因は `if (/vs\.?\|対/i.test(t) …) return 欧州サッカー…` のキャッチオール。今回の変更以前から存在 |
| **N-29** | サッカー判定の `Inter` が語境界なしで **"The International" にマッチ**。しかもサッカー判定がeスポーツ判定より前 | 実装 | 実測でマッチ確認。いまは `DIRECT_MAP` が先に効いて表示は正常 |
| **英語の流入** | `DIRECT_MAP` は23件、有効銘柄は128件で毎日入れ替わる。個別対応が追いつかない | 実装 | 永続性テスト中に `経済・市場予測: Will Brighton & Hove Albion FC…` が新規流入 |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | ユーザー | 唯一残るインフラ項目 |
| `existingRows` | ガードの既存行取得に `.range()` が無い。**N-20 と同じ形の静かな破綻** | 実装 | 現在 `events` は137行。上限に達すると保護が黙って効かなくなる |

### 翻訳まわりの構造（毎回ここで迷うので記録）

`sync_polymarket_cron.mjs` の `title_ja` 決定順：

1. `DIRECT_MAP[id]` — id 直指定（23件）。**最優先で他を全部飛ばす**
2. ルールベース（`Spread:` → サッカー、チーム名列挙 → MLB、`Cincinnati Open:` → テニス…）
3. `if (/vs\.?|対/i.test(t))` の**キャッチオール → 「欧州サッカー:」**（N-28 の原因）
4. カテゴリ別プレフィクス ＋ 英語原文（`経済・市場予測: Will …`）

そして **upsert 前のガード**：既存 `title_ja` に日本語があり `Will `/`Spread:` を含まなければ**保持**。
→ 良い翻訳は永続する。**悪い翻訳（`Will` を含む）は毎回作り直され、同じ悪い結果に固定される。**

### 検証で効いた手順（第5回）

- **永続性は同期をまたいで測る**：タイトルをスナップショット → cron を1回待つ → 突き合わせ。
  `updated_at` が進んだ件数（＝実際に書込対象になった件数）も一緒に見ないと「触られていないだけ」と区別できない
- **「その出力を作り得る箇所を全部数える」**：正規表現を1つ試して非マッチでも結論にしない。
  `grep -n "欧州サッカー"` で生成箇所を全部数えるのが先（第5回でここを飛ばして推論を誤った）
