...
| ID | 内容 | 提起 | 担当 |
|---|---|:--:|:--:|
| **N-59** | ハイドレーション後の h1 が崩れる（`/` はなし、`/forecast` はランクバッジ） | 第16回 | 実装 |
...
```
Core Web Vitals TTFB 430ms ／ FCP 636ms ／ LCP 872ms ／ CLS 0.027（いずれも良好圏）
アクセシビリティ alt 35/35 ／ ボタン名 140/140 ／ リンク 70/70 ／ lang=ja ／ 見出しの飛びなし
                 ただしハイドレーション後の h1 に問題あり（N-59）
確率の主語      7面すべてが一致。ソース49ファイル走査で直書き0件
投票            クライアント＋DB一意制約。埋め込みからの投票も記録される
API呼び出し     LiveTape の重複を解消（3回/周期 → 1回/周期）
ビルド          OGP画像とプリレンダーが同一のオッズを使う（N-58）
自己検証        33項目 全PASS
```
