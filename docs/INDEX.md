...
| ID | 内容 | 提起 | 担当 |
|---|---|:--:|:--:|
| **N-38** | 【世界観測銘柄】18件のうち **9件は答えが出せる**のに数字を出していない | 第10回 | 実装 |
| **Check #19 の限界** | 判定に**検査対象の関数自身**を使用／`dist` ではなくコミット済み辞書を参照／slug 直書き | 第10回 | 実装 |
| 期限整合性の検査 | 同期30分間隔に対し「締切超過 常時0件」を要求し、時間経過だけで赤くなる | 第10回 | 実装 |
| 閾値の不一致 | 「18回未満」の英語は `<20` | 第10回 | 実装 |
...
```
有効銘柄        77件  HTTP200 / og:title固有 / canonical自己参照 / JSON-LD / OGP  すべて 77/77
決着アーカイブ  63件  自己参照canonical ＋【決着・終了】＋ noindex, follow（ソフト404 0件）
sitemap         91URL（/market/ は77件・アーカイブの混入 0件）
n<3 ガード違反   0件 ／ 英語混入 0件 ／ 二重空白 0件
自己検証        21項目（実行時 20/21・落ちた1件は期限切れ3件による一過性）
```
