...
確率の主語      og:title・description・静的シェル・JSON-LD の4面が一致（36銘柄）
投票            クライアント側＋DB一意制約とも適用済み
                同一キーの2回目は 23505 で拒否／別キーとキー無しは通ることを実測
匿名キーの権限  events は UPDATE/DELETE ともに影響行0（RLSが防いでいる）
自己検証        31項目 全PASS
```
