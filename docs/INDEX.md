...
| **アプリ表示の回帰検査** | 分岐は `src/utils/probabilityLabel.ts` に集約したが、テスト基盤が無く自動検査ができない | 第13回 | 実装 |
...
```
sitemap         80URL   HTTP200 / 静的内部リンク / h1 / 本文  すべて 80/80
確率の主語      有効36件を市場IDで一意に突き合わせ
                  outcomes[0]=="Yes"  34件 → 「YES x%」で正しい
                  outcomes[0]!="Yes"   2件 → 主語を表示するよう修正
自己検証        29項目 全PASS
```
