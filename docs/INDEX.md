...
| ID | 内容 | 提起 | 担当 |
|---|---|:--:|:--:|
| **N-61** | ハイドレーション後に description / JSON-LD が既定値50%を捏造（n<3 の53銘柄） | 第17回 | 実装 |
| **本番404** | 未知URLが 200 でトップを返す | 第12回 | **ユーザー** |
| **P0-6** | Cloudflare Functions が実行されない | Phase 0 | **ユーザー** |
| B-3 ほか | `index.css` が1万行超 | 監査期 | 実装 |

## 2. 現在の本番の状態（第17回・実測）

```
見出し          h1×1 ／ 可視見出し43件 ／ 飛び0件（トップ・予測ハブ・埋め込みを是正）
n>=3 ガード     日本世論の割合を描画する12コンポーネントすべてに適用（母集団は導出）
Core Web Vitals TTFB 430ms ／ FCP 636ms ／ LCP 872ms ／ CLS 0.027
確率の主語      7面すべてが一致。ソース走査で直書き0件
投票            クライアント＋DB一意制約。埋め込みからの投票も記録される
API呼び出し     LiveTape の重複を解消（3回/周期 → 1回/周期）
自己検証        34項目 全PASS
既知の穴        ハイドレーション後の description / JSON-LD（N-61）
```
