...
| ID | 内容 | 提起 | 担当 | 状態 |
|---|---|:--:|:--:|:--:|
| **N-38** | 【世界観測銘柄】18件のうち 9件の選択肢型市場の個別オッズ解決 | 第10回 | 実装 | ✅ **解決済** |
| **Check #19** | 独立オラクル ＆ dist実走査への構造リファクタリング | 第10回 | 実装 | ✅ **解決済** |
| 期限整合性の検査 | 30分同期間隔に対応する1時間猶予ウィンドウ導入 | 第10回 | 実装 | ✅ **解決済** |
| 閾値の不一致 | Muskポスト数市場の「20回未満」への日本語タイトル是正 | 第10回 | 実装 | ✅ **解決済** |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML を返す） | Phase 0 | **ユーザー** | 待機中 |
| B-3 ほか | `index.css` が1万行超。構造リファクタリング（A-7 / F-3 / D-4 / F-1） | 監査期 | 実装 | 保留 |

---

## 2. 現在の本番の状態（第10回是正後・実測）

```
有効銘柄        74件  HTTP200 / og:title固有 / canonical自己参照 / JSON-LD / OGP  すべて 74/74
決着アーカイブ  63件  自己参照canonical ＋【決着・終了】＋ noindex, follow（ソフト404 0件）
sitemap         88URL（/market/ は74件・アーカイブの混入 0件）
n<3 ガード違反   0件 ／ 英語混入 0件 ／ 二重空白 0件
自己検証        22項目 全PASS（22/22 PASS 100%）
```
