...
| ID | 内容 | 提起 | 担当 |
|---|---|:--:|:--:|
| **本番404** | 未知URLが 200 でトップを返す。`404.html` は正しいが使われない（SPAフォールバック優先） | 第12回 | **ユーザー** |
| **P0-6** | Cloudflare Functions が実行されない（`/api/mcp` が SPA HTML） | Phase 0 | **ユーザー** |
| B-3 ほか | `index.css` が1万行超。構造リファクタリング（A-7 / F-3 / D-4 / F-1） | 監査期 | 実装 |

**実装側の未解決はありません。** 残る2件はどちらも Cloudflare Pages のダッシュボード操作です。
ソフト404は canonical が `/` を指すため重複ペナルティは回避されています（第12回・実測）。
...
sitemap         80URL
  HTTP 200                 80/80
  静的な内部リンク 1本以上   80/80  （中央値 11本／第12回で 1/79 → 80/80）
  h1 あり（4字以上）        80/80
  ナビを除く本文 80字以上    80/80
  noindex混入 0件 ／ canonical不一致 0件
匿名キーの書き込み  UPDATE・DELETE ともに影響行0（RLSが防いでいる）
自己検証          28項目 全PASS
```
...
| 12 | [phase1-verification-12-2026-08-24.md](phase1-verification-12-2026-08-24.md) | **N-49**（静的HTMLが本文0文字・リンク0本） | 78/79 → **80/80**。実装側の未解決が消滅 |
